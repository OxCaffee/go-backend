...
|SQL模版|SQL示例|加锁情况|说明|
|:---:|:---:|:---:|:---:|
|`select ... where pk = ... for update`|`select * from t where c1 = 20`|对c1=20主键记录加X锁，即`LOCK_X|LOCK_REC_NOT_GAP`|主键等值查询，记录存在|
|`select ... where pk = ... for update`|`select * from t where c1 = 15`|由于c1=15不存在，会在c1属于(10, 20)之间加间隙锁，即`LOCK_X|LOCK_GAP`，防止其他c1=15的记录插入|主键等值查询，记录不存在|
|