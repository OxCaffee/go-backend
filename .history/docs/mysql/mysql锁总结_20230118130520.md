...
|SQL模版|SQL示例|加锁情况|
|:---:|:---:|:---:|
|`select ... where pk >= ... for update`|`select * from t where c1 >= 20 for update`|对c1=20加上X锁，即`LOCK_X|LOCK_REC_NOT_GAP`，对c1=30和c1=40加上临键锁，即`LOCK_X|LOCK_ORDINARY`，加锁区间为`(20, 30], (30, 40]`，同时会对正无穷区间加上临键锁`(40, +∞)`