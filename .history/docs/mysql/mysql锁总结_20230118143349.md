...
|`select ... force index(...) where uk >= for update`|`select * from t force index(i_c2) where c2 >= 21 for update`|这种情况会强制走c2唯一索引，会给c2=`<21, 31, 41>`加上临键锁，即`LOCK_X|LOCK_ORDINARY`，加锁区间为`(-∞, 21], (21, 31], (31, 41], `