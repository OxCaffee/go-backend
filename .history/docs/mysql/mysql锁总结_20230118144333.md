...
|`select ... where uk <= ... for update`|`select * from t force index(i_c2) where c2 <= 21 for update`|会给c2=`<11, 21, 31>`加上临键锁，即`LOCK_X|LOCK_ORDINARY`, 加锁区间为`(-∞, 11], (11, 21], (21, 31]`，对c1=`<10, 20>加`