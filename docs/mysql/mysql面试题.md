...
`=`, `<=>`都被用来比较运算符两侧的操作数是否相等，如果两侧操作数相等就返回1，否则为0。**但是`<=>`可以安全的比较NULL类型，而`=`不行** 。

##  36. <a name='TEXTBLOB'></a>TEXT和BLOB的区别

TEXT只能存储字符类数据，而BLOB可以存储二进制数据

##  37. <a name='-1'></a>如何正确使用索引

* 为用于搜索，排序或者分组的列创建索引，而对于用作输出显示的列则不用创建索引
* 认真考虑列的基数：列的基数是指那么非重复值的个数，列的基数越高，索引的使用效果越好
* 索引短小值：尽量使用较小的数据类型作为索引，短小值可以让比较操作更快，可以让索引更小，减少磁盘IO请求，缓存的索引块可以容纳更多的键值
* 如果索引字符串的有不同的前缀，那么只需要为这个前缀添加索引即可
* 利用最左前缀，当创建包含n个列的复合索引的时候，实际上会创建n个专供MySQL使用的索引，复合索引相当于多个索引，因为索引中最左边的任意数据列集合都可用于匹配各个行。这样的集合被称为“最左前缀”
* 不要建立过多的索引
* 让参与比较的索引类型保持匹配，例如一个索引使用B树索引，那么参与比较的另外一个索引也应该使用B树索引
* 利用慢查询找出那些性能低劣的查询

###  37.1. <a name='InnoDB-1'></a>InnoDB使用的是什么索引

InnoDB使用的是聚簇索引，聚簇索引会把数据行和主键值存储在一起，其他的索引都是二级索引，即他们把主键值和二级索引值存储在一起，在二级索引中查找，会先得到主键值，然后再定位行，这意味着，主键值在每一个二级索引中都会重复出现
